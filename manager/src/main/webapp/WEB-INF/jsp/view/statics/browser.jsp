...
<div id="container" class="container">
    <div class="wrap-contents">
        <div class="page-control">
            <div class="row">
                <div class="col-xs-12">
                    <span class="page-title"> 접속 브라우저</span>
                </div>
            </div>
        </div>
        <div class="contents">
            ...
            <div class="card">
                                   ...
                                   <c:when test="${not empty list}">
                                       <c:forEach var="totalCnt" items="${list}" varStatus="status">
                                           <c:if test="${status.last}">
                                               <fmt:formatNumber value="${totalCnt.tot}" pattern="#,###"/>
                                           </c:if>
                                       </c:forEach>
                                   </c:when>
                                   <c:otherwise>
                ...
                <table class="table table-bordered table-vertical table-statistics">
                    ...
                    <thead>
                     ...
                     <tbody>
                       <c:forEach var="browser" items="${list}" varStatus="status">
                           <c:if test="${!status.last}">
                               <tr>
                                   <th scope="row">${browser.browser}</th>
                                   <td><fmt:formatNumber value="${browser.tot}" pattern="#,###"/></td>
                                   <td>${browser.per} %</td>
                               </tr>
                           </c:if>
                       </c:forEach>
...
<script type="text/javascript">
    ...
    function dateService(){
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/statics/browser",
            type: "POST",
    ...
    function monthService(){
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/statics/monthAccessBrowser",
            type: "POST",
    ...
    function yearService(){
      ...
      $.ajax({
            url: "${pageContext.request.contextPath}/statics/yearAccessBrowser",
            type: "POST",
