...
<div id="container" class="container">
    <div class="wrap-contents">
        <div class="page-control">
            <div class="row">
                <div class="col-xs-12">
                    <span class="page-title"> 매물 공유수</span>
                </div>
            </div>
        </div>
        <div class="contents">
                ...
                <table class="table table-bordered table-vertical table-statistics">
                    ...
                    <tbody>
                        <c:forEach var="share" items="${list}" varStatus="status">
                            <c:if test="${!status.last}">
                                <tr>
                                    <th scope="row">${share.platform}</th>
                                    <td><fmt:formatNumber value="${share.pc}" pattern="#,###"/></td>
                                    <td><fmt:formatNumber value="${share.mo}" pattern="#,###"/></td>
                                    <td>${share.per} %</td>
                                </tr>
                            </c:if>
                        </c:forEach>
...
<script type="text/javascript">
    ...
    function dateService(){
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/statics/share",
            type: "POST",
    ...
    function monthService(){
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/statics/monthShareCount",
            type: "POST",
    ...
    function yearService(){
      ...
      $.ajax({
            url: "${pageContext.request.contextPath}/statics/yearShareCount",
            type: "POST",
