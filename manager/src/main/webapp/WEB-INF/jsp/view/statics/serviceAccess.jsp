...
<div id="container" class="container">
    <div class="wrap-contents">
        <div class="page-control">
            <div class="row">
                <div class="col-xs-12">
                    <span class="page-title"> 서비스별 접속수</span>
                </div>
            </div>
        </div>
        <div class="contents">
                ...
                <table class="table table-bordered table-vertical table-statistics">
                    ...
                    <tbody>
                        <c:forEach var="dateList" items="${list}" varStatus="status">
                            <c:if test="${!status.last}">
                                <tr>
                                    <th scope="row">${dateList.menu}</th>
                                    <td><fmt:formatNumber value="${dateList.pc}" pattern="#,###"/></td>
                                    <td><fmt:formatNumber value="${dateList.mo}" pattern="#,###"/></td>
                                    <td>${dateList.per} %</td>
                                </tr>
                            </c:if>
                        </c:forEach>
...
<script type="text/javascript">
    ...
    function dateService(){
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/statics/serviceAccess",
            type: "POST",
    ...
    function monthService(){
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/statics/monthAccessService",
            type: "POST",
    ...
    function yearService(){
      ...
      $.ajax({
            url: "${pageContext.request.contextPath}/statics/yearAccessService",
            type: "POST",
