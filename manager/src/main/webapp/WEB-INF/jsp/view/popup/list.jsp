    ...
    <div class="wrap-contents">
        ...
        <div class="contents">
            <div class="card">
                <div class="tools">
                    <div class="row">
                        <form id="searchForm" action="${pageContext.request.contextPath}/popup/list" method="POST">
                        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
                        ...
                        <form id="PopupForm">
                            <c:forEach items="${list}" var="item" varStatus="status">
                                <tr>
                                    ...
                                    <td class="text-left">
                                        <a name="title" href="${pageContext.request.contextPath}/popup/detail/${item.popupSeq}">${item.title}</a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="onoffswitch">
                                            ...
                                            <input type="hidden" name="popupSeq" value="${item.popupSeq}">
...
$(document).on("click","#registe",function(){
    location.href = "${pageContext.request.contextPath}/popup/registeForm";
});

//편집버튼 이동
function edit(popupSeq){
    $("#subForm").attr("action", "${pageContext.request.contextPath}/popup/detail");
    $("#subForm").attr("method", "POST");
    $("#subForm").find("input[name='popupSeq']").val(popupSeq);
    $("#subForm").submit();
}
...
$(document).on("click","button[name='applyButton']",function(){
    var td=$(this).closest("td");
    var popupSeq=$(td).find("input[name='popupSeq']").val();
    if($(td).find("input[name='isVisibled']").prop("checked")){
        var expsYn='Y';
        if(Number("${popupExpsYCount}") >= 1){
    ...
    $.ajax({
        url : "${pageContext.request.contextPath }/popup/popupExpsYnUpdate",
        type : "POST",
        data : {popupSeq:popupSeq ,expsYn:expsYn},
        dataType : "text",
...
function search(){
    $.ajax({
        url : "${pageContext.request.contextPath}/popup/list",
        type: "POST",
